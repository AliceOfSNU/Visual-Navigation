...
int main(int argc, char **argv)
{
  ...
  ros::Publisher pose_pub =
      nh.advertise<geometry_msgs::PoseStamped>("orb_pose", 100);
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,
  ...
  if(do_rectify) {
    ...
    mTcw = mpSLAM->TrackStereo(imLeft, imRight,
                               cv_ptrLeft->header.stamp.toSec()).clone();
  } else {
    mTcw = mpSLAM->TrackStereo(cv_ptrLeft->image,
                               cv_ptrRight->image,
                               cv_ptrLeft->header.stamp.toSec()).clone();
  }
}
